...
int main(int argc, char** argv)
{
  ...
  for(int i=0;i<10;i++){Tab[i]=i;}
      ...
      int nbr = getTaskParams("user/non_rt.desc",ArrayOfTasks);
      cout << "Le programmer a détecté  " << nbr << " Taches" << endl;
      affiche(ArrayOfTasks,nbr);
      detect_task("user/non_rt.desc",&myfile, ArrayOfTasks,nbr);
  ...
  for (int i=0;i<nbr;i++){
      ...
      int res = extractMethod(FunctionName,"user/non_rt.c","tmp_c");
      if (res == 0){
